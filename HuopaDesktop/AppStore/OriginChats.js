async function loop() {
    let ws;
    let serverList;
    serverList = await huopaAPI.fs.getFile("/home/applications/OriginChats/serverlist.json");
    let serverToOpen = await huopaAPI.fs.getFile("/home/applications/OriginChats/currentServerOpen.txt");
    let extraConfig = await huopaAPI.fs.getFile("/home/applications/OriginChats/config.json");;
    if (!serverList) {
        firstOpen = true;
        serverList = '["chats.mistium.com"]';
        await huopaAPI.fs.writeFile("/home/applications/OriginChats/serverlist.json", "file", serverList);
        serverToOpen = "0"
        await huopaAPI.fs.writeFile("/home/applications/OriginChats/currentServerOpen.txt", "file", serverToOpen);
    }
    if (!extraConfig) {
        extraConfig = '{"messageLogger":false,"notifications":false}';
        await huopaAPI.fs.writeFile("/home/applications/OriginChats/config.json", "file", extraConfig);
    }
    ...
    async function connectError() {
        const icon = await huopaAPI.fs.getFile(`/home/applications/OriginChats/serverIconStore/${url}`);
        if (!icon) {
            serverList.splice(serverToOpen, 1);
            await huopaAPI.fs.writeFile("/home/applications/OriginChats/serverlist.json", "file", JSON.stringify(serverList));
        }
        serverToOpen = "0"
        await huopaAPI.fs.writeFile("/home/applications/OriginChats/currentServerOpen.txt", "file", serverToOpen);
        const errorMsg = await huopaAPI.createElement("h2");
        await huopaAPI.window.setTitle("OriginChats - Failed to connect to server");
    ...
    let auth, appState = "auth", ratelimited, userObj, userData, server, messageLengthLimit, userColors, roles, openedChannel, userList, channelMsgs, channelList, validatorKey, loading, editingMessage = false; messageTable = {}, delAllowed = false, loading2 = false;
    ...
    await huopaAPI.fs.writeFile("/home/applications/OriginChats/currentServerOpen.txt", "file", "0");
        ...
        if (appState === "auth") {
            ...
            switch (data.cmd) {
                case "handshake":
                    ...
                    await huopaAPI.fs.writeFile(`/home/applications/OriginChats/serverIconStore/${url}`, "file", server.icon);
                    await huopaAPI.window.setTitle(`OriginChats - ${server.name}`);
                    loginPrompt();
                    break;
                case "auth_success":
    ...
    async function createMainUI() {
        try {
            ...
            for (const serverI in serverList) {
                ...
                const icon = await huopaAPI.fs.getFile(`/home/applications/OriginChats/serverIconStore/${serverUrl}`);
                ...
                if (icon) {
                    await setAttrs(serverIcon, {
                        ...
                        "onclick": async () => {
                            await huopaAPI.fs.writeFile("/home/applications/OriginChats/currentServerOpen.txt", "file", serverI)
                        ...
                        if (key === "Enter") {
                            ...
                            if (value) {
                                if (serverList.includes(value)) {
                                    await huopaAPI.fs.writeFile("/home/applications/OriginChats/currentServerOpen.txt", "file", serverList.indexOf(value))
                                } else {
                                    serverList.push(value);
                                    await huopaAPI.fs.writeFile("/home/applications/OriginChats/serverlist.json", "file", JSON.stringify(serverList));
                                    await huopaAPI.fs.writeFile("/home/applications/OriginChats/currentServerOpen.txt", "file", serverList.length - 1)
                                }
    ...
    async function registerFuncs() {
            ...
            for (channel of channelList) {
                ...
                await huopaAPI.setAttribute(channelDiv, "onclick", async() => {
                    try {
                        ...
                        await huopaAPI.window.setTitle(`OriginChats - #${channelSave.name} - ${server.name}`);
                        loading = true;
        ...
        wsHandlers.set("message_new", async(data) => {
            ...
            if (msg.user !== userObj.username && extraConfig.notifications) {
                huopaAPI.system.createNotification(`${msg.user} - #${data.channel} - ${server.name}`, msg.content);
            }
